...
export default async function HomePage() {
  ...
  if (me?.lastNoteId) {
    // Confirm the note still exists (it may have been deleted since it was
    // last opened) before sending the user to a dead link. This request
    // also naturally re-marks it as "last opened" via the GET /notes/{id}
    // endpoint, which is harmless here.
    const noteRes = await fetch(`${API_BASE}/notes/${me.lastNoteId}`, {
      headers: authHeader,
      cache: 'no-store',
    })

    if (noteRes.ok) {
      redirect(`/notes/${me.lastNoteId}`)
    }
    // 404 (deleted) or any other failure — fall through to the gallery.
  }

  redirect('/notes') // gallery view — no last note, or it no longer exists
}